package io.mixeway.scanmanager.integrations.fortify.apiclient;
...
import io.mixeway.scanmanager.integrations.fortify.model.*;
...
public class FortifyApiClient implements CodeScanClient, SecurityScanner {
	...
	FortifyApiClient(VaultHelper vaultHelper, ScannerRepository scannerRepository, VulnTemplate vulnTemplate,
	...
	//SSC Generation
	@Override
	public boolean initialize(Scanner scanner) throws JSONException, ParseException, CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException, KeyStoreException, IOException {
	...
	private boolean generateToken(Scanner scanner) throws CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException, KeyStoreException, IOException, JSONException, ParseException {
	...
	@Override
	public void loadVulnerabilities(Scanner scanner, CodeGroup codeGroup, String urlToGetNext, Boolean single, CodeProject codeProject, List<ProjectVulnerability> codeVulns) throws URISyntaxException, ParseException, JSONException {
		try {
	...
	private void saveVulnerabilities(CodeGroup codeGroup, List<FortifyVuln> fortifyVulns, CodeProject cp, Scanner scanner, List<ProjectVulnerability> oldVulns) throws JSONException, CertificateException, ParseException, NoSuchAlgorithmException, KeyManagementException, UnrecoverableKeyException, KeyStoreException, IOException, URISyntaxException {
	...
	private ProjectVulnerability createDescriptionAndState(String instanceId, Long id, int versionid, Scanner scanner, ProjectVulnerability codeVuln) throws ParseException, CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException, JSONException, KeyStoreException, IOException, URISyntaxException {
...
//			if (bugTracker.isPresent() && StringUtils.isNotBlank(codeVuln.getTicketId())) {
...
//		}
//	}
	private String getCodeSnippet(Scanner scanner, int versionid, String fullFileName, int lineNumber) throws CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException, KeyStoreException, IOException, JSONException, ParseException {
		String codeSnippet = "";
		CodeRequestHelper codeRequestHelper = prepareRestTemplate(scanner);
	...
	private boolean verifyCloudScanJob(CodeGroup cg) throws ParseException, CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException, JSONException, KeyStoreException, IOException {
		try {
			Scanner scanner = scannerRepository.findByScannerType(scannerTypeRepository.findByNameIgnoreCase(Constants.SCANNER_TYPE_FORTIFY)).get(0);
			CodeRequestHelper codeRequestHelper = prepareRestTemplate(scanner);
	...
	public boolean canProcessRequest(CodeGroup cg) {
		Optional<Scanner> fortify = scannerRepository.findByScannerType(scannerTypeRepository.findByNameIgnoreCase(Constants.SCANNER_TYPE_FORTIFY_SCC)).stream().findFirst();
